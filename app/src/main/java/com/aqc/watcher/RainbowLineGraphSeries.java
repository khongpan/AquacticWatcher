    ...
    public void draw(GraphView graphView, Canvas canvas, boolean isSecondScale) {
        ...
        while (values.hasNext()) {
            ...
            if (i > 0) {
                ...
                mPath.reset(); //remove for speed
                mPath.moveTo(first_X, first_Y); //remove for speed
...
/*
        ...
        for (int i =0;i< mLevelColor.length;i++) {
            if (value.getY() >= mLevelColor[i].getX()) {
                paint.setColor((int) mLevelColor[i].getY());
                break;
            }
...
*/
    ...
    public void setFlatLine(boolean flat_line) {
        mFlatLine = flat_line;
    }
}